...
# Nieuw: duidelijk volgens jouw wens
# Links = Beschikbaar (nu opneembaar) = start + gestort + gerealiseerde winst - open inzetten
available_balance = start_amt + deposits_total + realized_profit_total - open_stake_total
# Rechts = Openstaande inzetten = som van stakes met outcome NaN
open_bets_amount = open_stake_total
...
<div style="padding:16px; background:#0f172a; color:#fff; border-radius:14px; margin-bottom:12px;">
  <div style="display:flex; gap:16px; justify-content:space-between; align-items:center; flex-wrap:wrap;">
    <div style="flex:1; min-width:240px; text-align:center;">
      <div style="font-size:14px; opacity:.85;">Beschikbaar saldo (nu opneembaar)</div>
      <div style="font-size:44px; font-weight:800; line-height:1; margin:6px 0 2px;">€ {available_balance:,.2f}</div>
    </div>
    <div style="flex:1; min-width:240px; text-align:center;">
      <div style="font-size:14px; opacity:.85;">Openstaande inzetten</div>
      <div style="font-size:44px; font-weight:800; line-height:1; margin:6px 0 2px;">€ {open_bets_amount:,.2f}</div>
    </div>
  </div>
...
with st.expander("Filter op datum", expanded=False):
    ...
    st.write(f"Geselecteerd: **{len[df_filtered]}** bets" if hasattr(df_filtered, "__len__") else "")
    if len(df_filtered):
